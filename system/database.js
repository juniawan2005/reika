module.exports = (m) => {
   ...
   if (setting) {
      ...
      if (!('sk_pack' in setting)) setting.sk_pack = 'Bot WA'
      if (!('sk_author' in setting)) setting.sk_author = '@reika.bot'
      ...
      if (!('owners' in setting)) setting.owners = ['6281998903280']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = 'Hai kak, +tag\nSaya adalah sistem otomatis (WhatsApp Bot) yang dapat membantu untuk melakukan sesuatu, mencari dan mendapatkan data/informasi hanya melalui WhatsApp.\n\n◦ *Database* : +db\n◦ *Library* : Baileys v6.0.1\n◦ \n\nJika Anda menemukan kesalahan atau ingin meningkatkan paket premium hubungi pemilik bro.'
      if (!isNumber(setting.menuStyle)) setting.menuStyle = 4
      if (!('cover' in setting)) setting.cover = 'https://i.ibb.co/T47hHyq/reika.png'
      if (!('link' in setting)) setting.link = 'https://chat.whatsapp.com/H3Y6fplgXSR4V6Nr7pAhWr'
   } else {
      global.db.setting = {
         autodownload: true,
         chatbot: true,
         ...
         online: true,
         onlyprefix: '+',
         owners: ['6281998903280'],
         lastReset: new Date * 1,
         msg: 'Halo kak, +tag\nSaya adalah sistem otomatis (WhatsApp Bot) yang dapat membantu untuk melakukan sesuatu, mencari dan mendapatkan data/informasi hanya melalui WhatsApp.\n\n◦ *Database* : +db\n◦ *Library* : Baileys v6.0.1\n◦ \n\nJika Anda menemukan kesalahan atau ingin meningkatkan paket premium hubungi pemilik bro.',
         menuStyle: 3,
         cover: 'https://i.ibb.co/T47hHyq/reika.png',
         link: 'https://chat.whatsapp.com/H3Y6fplgXSR4V6Nr7pAhWr'
      }
   }
}
